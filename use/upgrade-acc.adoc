...
NOTE: Astra Control Center 업그레이드를 위해서는 Astra 버전을 변경해야 합니다. 레지스트리 경로는 에서 이미지를 푸시한 레지스트리 경로와 일치해야 합니다 ,이전 단계.
...
NOTE: 이미 이(가) 있어야 합니다  up acc-helm-repo to upgrade Traefik and Cert-manager,acc-helm-repo 설정 다음 절차를 완료하기 전에
...
NOTE: 을(를) 이미 완료해야 합니다  Traefik service using acc-helm-repo,Traefik 업데이트 및  up acc-helm-repo to upgrade Traefik and Cert-manager,Helm에 acc-helm-repo 추가 다음 절차를 완료하기 전에
